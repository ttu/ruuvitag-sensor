...
def write_to_influxdb(received_data):
    ...
    fields = {}
    fields['temperature'] = received_data[1]['temperature'] if ('temperature' in received_data[1]) \
        else None
    ...
    fields['accelerationX'] = received_data[1]['acceleration_x'] if ('acceleration_x' in
                                                                     received_data[1]) else None
    fields['accelerationY'] = received_data[1]['acceleration_y'] if ('acceleration_y' in
                                                                     received_data[1]) else None
    fields['accelerationZ'] = received_data[1]['acceleration_z'] if ('acceleration_z' in
                                                                     received_data[1]) else None
    fields['batteryVoltage'] = received_data[1]['battery'] / 1000.0 \
        if ('battery' in received_data[1]) else None
    fields['txPower'] = received_data[1]['tx_power'] if ('tx_power' in received_data[1]) else None
    fields['movementCounter'] = received_data[1]['movement_counter'] \
        if ('movement_counter' in received_data[1]) else None
    fields['measurementSequenceNumber'] = received_data[1]['measurement_sequence_number'] \
        if ('measurement_sequence_number' in received_data[1]) else None
...
).subscribe(lambda x: x.pipe(ops.sample(interval_in_s)).subscribe(try_write_to_influxdb))
